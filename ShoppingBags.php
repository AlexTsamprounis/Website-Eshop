...
<html>
<head>
    <title>AT.COLLECTION | Shopping Bags</title>
    <link href="test2.css" rel="stylesheet" type="text/css">
    <script>
        // Αυτό διασφαλίζει ότι όλες οι σελίδες χρησιμοποιούν το ΙΔΙΟ καλάθι
        const currentUserEmail = "<?php echo $_SESSION['user']['email'] ?? 'guest'; ?>";
    </script>
    <script src="cart.js" defer></script>
        <script>  /* ΠΡΟΣΘΕΣΕ ΑΥΤΟ ΕΔΩ */
    ...
    <main class="main-content page-home">
        <section class="best-sellers" id="category">
            <div class="best-sellers__header">
                <h2 class="best-sellers__title">Shopping Bags Collection!!</h2>
            </div>
            
            <div class="products-flex">
                <?php
                // Δυναμική λήψη προϊόντων από τη βάση
                $query = "SELECT * FROM products WHERE category = 'shopping'"; 
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        <article class="product-card">
                            <img class="product-card__img" src="<?php echo $row['image_path']; ?>" alt="<?php echo $row['name']; ?>">
                        ...
                        <button onclick="addToCart('<?php echo $row['name']; ?>', <?php echo $row['price']; ?>)" 
                                class="auth-button" 
                                style="width:100%; cursor:pointer; background-color:#ff9d00; border:none; padding:10px;">
                            Add to Cart
                        </button>
                    ...
                    echo "<p style='color:white;'>Δεν υπάρχουν διαθέσιμα προϊόντα.</p>";
                }
                ?>
            </div>
